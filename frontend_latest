...
export default function Home() {
  ...
  const services = [
    {
      ...
      description: 'Conducts, records, procfors, and evaluates in a structured video interview.',
  ...
  const handleLoginSubmit = async (e) => {
    ...
    try {
      ...
      if (data.role === 'hr_manager' || loginType === 'admin') {
        navigate('/jd');
      } else {
        navigate('/assessment');
      }
  ...
  return (
    <div className="h-screen bg-gradient-to-br from-sky-50 via-white to-green-50 font-sans text-sky-950 relative overflow-hidden flex flex-col selection:bg-sky-200">
      ...
      <section className="flex-1 max-w-7xl w-full mx-auto px-8 pb-8 relative z-10 flex flex-col min-h-0">
        {/* Grid - perfectly spaced to fill remaining height safely */}
            ...
            <div className="w-full md:w-1/2 p-8 md:p-12 flex flex-col justify-center bg-white relative overflow-y-auto">
              ...
              <form onSubmit={handleLoginSubmit} className="space-y-4">
                ...
                <p className="text-center text-sm text-gray-500 font-medium mt-6">
                  Don't have an account? <span className="text-green-600 font-bold hover:underline cursor-pointer">Sign Up</span>
                </p>
              </form>
            </div>
          </div>
